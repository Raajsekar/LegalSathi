...
import { Plus, LogOut, Upload, Copy, FileText, BookOpen } from "lucide-react";
...
export default function Chat() {
  ...
  const [task, setTask] = useState("summarize"); // summarize | contract | explain
  const [loading, setLoading] = useState(false);
  const [chats, setChats] = useState([]); // list of chat items
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // prepend but keep history saved on backend
  ...
  // Reset conversation: create a new chat (does not delete history on backend)
  const handleResetConversation = () => {
    // create a new blank activeChat placeholder
    const placeholder = {
      ...
      <main className="flex-1 flex flex-col">
        ...
        <header className="px-6 py-4 border-b border-gray-800 flex items-center justify-between bg-[#0f1012]">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => { /* quick library nav */ window.location.pathname = "/library"; }}
        ...
        <section className="flex-1 overflow-y-auto p-6" ref={scrollRef}>
          {!activeChat ? (
            ...
            <article className="max-w-3xl mx-auto">
              <div className="mb-4 text-sm text-blue-400 text-right">{activeChat.message}</div>
              ...
              <div className="flex items-center gap-3 mt-3">
                {activeChat.pdf_url && (
                  <a href={activeChat.pdf_url.startsWith("http") ? activeChat.pdf_url : `${API_BASE}${activeChat.pdf_url}`}
        ...
        <footer className="p-4 border-t border-gray-800 bg-[#0f1012]">
          <div className="max-w-6xl mx-auto flex items-center gap-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={() => { setMessage(""); setFile(null); setFileName(""); }}
